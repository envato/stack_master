...
Given(/^I stub CloudFormation validate calls to pass validation$/) do
  expect(StackMaster.cloud_formation_driver).to receive(:validate_template).and_return(true).once
end

Given(/^I stub CloudFormation validate calls to fail validation with message "([^"]*)"$/) do |message|
  expect(StackMaster.cloud_formation_driver).to receive(:validate_template).and_raise(Aws::CloudFormation::Errors::ValidationError.new('', message)).once
end
