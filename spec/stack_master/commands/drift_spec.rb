RSpec.describe StackMaster::Commands::Drift do
  ...
  before do
    options.timeout = 10
    allow(StackMaster)
      .to receive(:cloud_formation_driver)
      .and_return(cf)
    allow(cf)
      .to receive(:detect_stack_drift)
      .and_return(detect_stack_drift_response)
    allow(cf)
      .to receive(:describe_stack_drift_detection_status)
      .and_return(describe_stack_drift_detection_status_response)
    allow(cf)
      .to receive(:describe_stack_resource_drifts)
      .and_return(describe_stack_resource_drifts_response)
