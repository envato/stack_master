RSpec.describe StackMaster::SparkleFormation::CompileTime::ParametersValidator do
  ...
  before(:each) do
    allow(StackMaster::SparkleFormation::CompileTime::ValueValidatorFactory)
      .to receive(:new)
      .and_return(value_validator_factory)
    allow(value_validator_factory)
      .to receive(:build)
      .and_return([value_validator])
    allow(value_validator)
      .to receive(:validate)
    allow(value_validator)
      .to receive(:is_valid)
      .and_return(true)
  ...
  describe '#validate' do
    it('should initialise the ValueValidatorFactory') do
      expect(StackMaster::SparkleFormation::CompileTime::ValueValidatorFactory)
        .to receive(:new)
        .with(:ip, { type: :string }, '10.0.0.0')
      subject.validate
    end
