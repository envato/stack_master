  ...
  describe '#resolve' do
    ...
    context 'when we have set OP_SESSION_ environment' do
      ...
      context 'when retrieving a login password' do
        it 'returns the login password' do
          ...
          allow_any_instance_of(described_class)
            .to receive(:`)
            .with("op get item --vault='Shared' 'password title' 2>&1")
            .and_return(login_password.to_json)
      ...
      context 'when retrieving a secureNote' do
        it 'returns the secureNote' do
          ...
          allow_any_instance_of(described_class)
            .to receive(:`)
            .with("op get item --vault='Shared' 'note title' 2>&1")
            .and_return(secureNote.to_json)
    ...
    context 'when we have not set OP_SESSION_ environment' do
      context 'when retrieving a password' do
        it 'should raise error when ENV not set' do
          allow(ENV)
            .to receive_message_chain(:keys, :grep)
            .with(/OP_SESSION_\w+$/)
            .and_return([])
          allow_any_instance_of(described_class)
            .to receive(:`)
    ...
    context 'when items are not found' do
      ...
      it 'we return an error' do
        ...
        allow_any_instance_of(described_class)
          .to receive(:`)
          .with("op get item --vault='Shared' 'password title' 2>&1")
          .and_return('[LOG] 2018/03/26 09:56:02 (ERROR) Vault Shared not found.')

        expect { resolver.resolve(the_password) }
          .to raise_error(
            StackMaster::ParameterResolvers::OnePassword::OnePasswordNotFound,
            'Failed to return item from 1password, (ERROR) Vault Shared not found.'
          )
      end
    end
    ...
    context 'when returned value is invalid' do
      ...
      it 'we return an error' do
        ...
        expect { resolver.resolve(the_password) }
          .to raise_error(
            StackMaster::ParameterResolvers::OnePassword::OnePasswordInvalidResponse,
            /Failed to parse JSON returned, {key: value }:/
          )
      end
    end
  end
