RSpec.describe StackMaster::ParameterResolvers::LatestContainer do
  ...
  context 'when matches are found' do
    ...
    it 'returns the latest one' do
      expect(resolver.resolve({ 'repository_name' => 'foo' }))
        .to eq '012345678910.dkr.ecr.us-east-1.amazonaws.com/foo@sha256:deadbeef'
    end
  end
  ...
  context 'when a tag is passed in' do
    ...
    context 'when image exists' do
      it 'returns the image with the production tag' do
        expect(resolver.resolve({ 'repository_name' => 'foo', 'tag' => 'production' }))
          .to eq '012345678910.dkr.ecr.us-east-1.amazonaws.com/foo@sha256:decafc0ffee'
      end
    end

    context 'when no image exists for this tag' do
      it 'returns nil' do
        expect(resolver.resolve({ 'repository_name' => 'foo', 'tag' => 'nosuchtag' }))
          .to be_nil
      end
    end
  end
  ...
  context 'when there are multiple pages' do
        ...
        :describe_images, [
          ...
          },
          {
            ...
            image_details: [
              {
                registry_id: '012345678910',
                image_digest: 'sha256:badf00d',
                image_pushed_at: Time.utc(2015, 1, 4, 0, 0),
                image_tags: ['v3']
              },
              {
                registry_id: '012345678910',
                image_digest: 'sha256:d15ea5e',
                image_pushed_at: Time.utc(2015, 1, 5, 0, 0),
                image_tags: ['v4']
              }
            ]
          }
    ...
    it 'takes all pages into account' do
      expect(resolver.resolve({ 'repository_name' => 'foo' }))
        .to eq '012345678910.dkr.ecr.us-east-1.amazonaws.com/foo@sha256:d15ea5e'
    end
  end
end
